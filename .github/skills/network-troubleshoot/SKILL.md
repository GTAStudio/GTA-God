...
description: Systematic, safety-first network troubleshooting for developers. Use when diagnosing connection failures, DNS issues, TLS/certificate errors, proxy/VPN problems, package-registry or Git network failures, or HTTP error codes (403/407/5xx). Keeps diagnostics read-only and target-scoped; never bypasses TLS or changes system network/proxy state without explicit user approval.
---

# Network Troubleshoot

Use this skill as a concise decision guide for developer network failures. Keep diagnostics safe, target-scoped, and read-only. Do not turn this into an automated remediation toolkit.
