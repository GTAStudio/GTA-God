...
pub struct Config {
    ...
    /// ClientHello 完成后等待转发容量的最长时间（秒，0 表示不等待、立即卸载）。
    #[serde(default = "default_relay_queue_timeout")]
    pub relay_queue_timeout_secs: u64,
    /// 已建立转发连接的双向字节空闲超时（秒，0 表示禁用）。默认禁用，
    /// 由 TCP keepalive 回收失联端点，避免误杀合法的空闲隧道。
    #[serde(default = "default_relay_idle_timeout")]
    pub relay_idle_timeout_secs: u64,
    /// Linux TCP 已发送但未确认数据的最长存活时间（秒，0 表示使用系统默认）。
    /// 与空闲超时不同，它只约束网络黑洞或长期零窗口造成的未确认数据。
    #[serde(default = "default_tcp_user_timeout")]
...
impl Config {
    ...
    fn validate(&self) -> anyhow::Result<()> {
        ...
        if self.relay_queue_timeout_secs > 300 {
            anyhow::bail!("relay_queue_timeout_secs 不能超过 300 秒");
        }
        if self.tcp_user_timeout_secs > 24 * 3600 {
            anyhow::bail!("tcp_user_timeout_secs 不能超过 86400 秒");
        }
...
fn default_relay_queue_timeout() -> u64 {
    5
}
...
fn default_tcp_user_timeout() -> u64 {
    300
}
...
mod tests {
    ...
    #[test]
    fn relay_idle_timeout_is_disabled_by_default() {
        let cfg = load_str(
            r#"{
        ...
        assert_eq!(cfg.relay_idle_timeout_secs, 0);
    }

    #[test]
    fn tcp_user_timeout_has_bounded_default() {
        let cfg = load_str(
            r#"{
                "listen": "127.0.0.1:443",
                "default_upstream": "127.0.0.1:8443"
        ...
        assert_eq!(cfg.tcp_user_timeout_secs, 300);
    }
