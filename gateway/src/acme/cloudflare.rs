...
impl Cloudflare {
    ...
    pub async fn delete_txt(&self, zone_id: &str, record_id: &str) -> anyhow::Result<()> {
        ...
        if cloudflare_record_is_missing(&body) {
            return Ok(());
        }
...
fn cloudflare_record_is_missing(body: &serde_json::Value) -> bool {
    body["errors"].as_array().is_some_and(|errors| {
        errors
            .iter()
            .any(|error| error["code"].as_u64() == Some(81044))
    })
}
...
#[cfg(test)]
mod tests {
    use super::cloudflare_record_is_missing;
    use serde_json::json;

    #[test]
    fn missing_dns_record_error_is_idempotent_success() {
        assert!(cloudflare_record_is_missing(&json!({
            "success": false,
